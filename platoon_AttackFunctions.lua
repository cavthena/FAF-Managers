...
local function PlatoonAlive(platoon)
    ...
    if brain and brain.PlatoonExists then
        if brain:PlatoonExists(platoon) then
            return true
        end
    end

    -- UnitSpawner-created platoons can lose their platoon registration while
    -- their units are still alive and commandable. Fall back to checking unit
    -- membership so long-running attack threads (e.g. DefensePatrol) continue.
...
-- Returns true only when every alive unit in the platoon is within radiusSq
-- of pos. Used by DefensePatrol to keep the platoon grouped: the next
-- waypoint is not issued until the entire platoon has consolidated at the
-- current one, preventing units from stringing out along the patrol route.
local function HasAllArrived(platoon, pos, radiusSq)
    local units = PlatoonUnits(platoon)
...
local function WaitForArrival(platoon, pos, options)
    ...
    -- ArriveAll: wait until every alive unit has reached the radius, not just one.
    -- Used by DefensePatrol to regroup the platoon at each waypoint before advancing.
    local arrivedFn = (options and options.ArriveAll) and HasAllArrived or HasArrived
    while PlatoonAlive(platoon) and elapsed < timeout do
        ...
        if arrivedFn(platoon, pos, radiusSq) then
            return true
        end
...
local function AirCircleAt(platoon, markerPos, options)
    ...
    -- Use IssuePatrol so units loop the orbit continuously rather than
    -- completing one pass and going idle. Points loop i = 0..points-1 so
    -- the start and end positions are not duplicated (cos/sin are periodic).
    local r = options.WaitCircleRadius or options.CircleRadius or 48
...
local function WaitAtMarker(platoon, marker, options)
    ...
    -- Orbit uses IssuePatrol (continuous loop), so only re-issue if interrupted.
    local orbitRefresh = options.OrbitRefresh or 30

    if domain == 'AIR' then
        local orbitRadius      = options.WaitCircleRadius or options.CircleRadius or 48
        -- "Near marker" means the platoon has reached the orbit ring area.
        -- Use orbit radius + 20 so units transitioning onto the ring are counted.
        local approachRadiusSq = (orbitRadius + 20) * (orbitRadius + 20)

        if not HasArrived(platoon, pos, approachRadiusSq) then
            -- Platoon is still en route: issue a direct move to the marker center.
            -- Do NOT call AirCircleAt here; it would call IssueClearCommands and
            -- cancel this move order before the platoon can reach the marker.
            if not waitState.lastMoveOrder or (now - waitState.lastMoveOrder) >= moveRefresh then
                MovePlatoonTo(platoon, pos, options)
                waitState.lastMoveOrder  = now
                waitState.lastOrbitOrder = nil  -- reset so orbit re-arms on arrival
            end
        else
            -- Platoon has reached the marker area: set up the continuous patrol orbit.
            -- Re-issue periodically in case combat interrupted the patrol commands.
            if not waitState.lastOrbitOrder or (now - waitState.lastOrbitOrder) >= orbitRefresh then
...
-- Always yields at least one interval before the first check so the engine has
-- time to register the Upgrading state after IssueUpgrade is called. Without
-- this initial yield the function can return immediately (upgrade not yet
-- registered) and the caller enters a zero-yield tight loop.
