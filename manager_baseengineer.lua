--[[
================================================================================
 Base Engineer Manager -- Created by Cavthena
================================================================================
...
Usage
    ...
    local baseHandle = BaseManager.Start{
        ...
        empty        = false,                                -- optional; if true, skip spawning structGroups and engineers on start
...
-- Clamp pos so it is no farther than radius from center.
local function _ClampPosToRadius(pos, center, radius)
    local dx = pos[1] - center[1]
    local dz = pos[3] - center[3]
    local d  = math.sqrt(dx * dx + dz * dz)
    if d <= radius then return pos end
    local scale = radius / d
    return { center[1] + dx * scale, pos[2], center[3] + dz * scale }
end
...
function M:_TickBaseDefense(u, id, enemies)
    ...
    if healthRatio <= 0.25 then
        ...
        -- Never retreat outside the base perimeter.
        if self.basePos then
            retreatPos = _ClampPosToRadius(retreatPos, self.basePos, self.params.radius or 60)
        end
...
local function NormalizeEngineerParams(p)
    ...
    return {
        ...
        empty        = p.empty and true or false,
